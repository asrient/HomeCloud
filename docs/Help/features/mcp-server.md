...
HomeCloud includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI assistants interact with your devices through HomeCloud's workflow scripting environment.
...
The MCP server currently provides one tool:

### `execute_script`

Executes a JavaScript script in HomeCloud's workflow environment and returns the result. Max execution time is 5 minutes.
...
- `debugLog` — Console output from the script (if any)

Through this tool, the AI agent can use the full workflow scripting API, including:

- **File operations** — Read, write, and browse files on any connected device
- **System info** — Get device details, storage info, and more
- **Secrets** — Securely store and retrieve API keys or credentials
- **Cross-device access** — Interact with remote HomeCloud devices by fingerprint

## Security

- **Localhost only** — The server binds to `127.0.0.1` and cannot be reached from other machines.
- **Isolated execution** — Scripts run in isolated worker threads with timeouts.
- **Permission prompts** — The AI agent can request permission before performing sensitive operations, and you approve or deny from the chat UI.
