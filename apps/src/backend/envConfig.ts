...
export const StorageTypeMeta: {
    ...
    [StorageType.WebDav]: {
        name: 'WebDAV',
        allowedAuthTypes: [StorageAuthType.Basic, StorageAuthType.Digest, StorageAuthType.None],
...
export const implementedStorageTypes = [
    StorageType.WebDav,
    StorageType.Google,
];
...
class EnvConfig {
    ...
    constructor(config: SetupParams) {
        ...
        this.API_BASE_URL = config.apiBaseUrl || joinUrlPath(config.baseUrl, '/api/');
        this.WEB_BUILD_DIR = config.webBuildDir;
        this.PROFILES_CONFIG = config.profilesPolicy;
        this.SECRET_KEY = config.secretKey;
        this.ENABLED_STORAGE_TYPES = !!config.disabledStorageTypes ? implementedStorageTypes.filter(t => !config.disabledStorageTypes?.includes(t)) : implementedStorageTypes;
