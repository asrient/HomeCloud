...
export class LocalFsDriver extends FsDriver {
  ...
  async toRemoteItem(filePath: string): Promise<RemoteItem> {
    const stat = await fs.stat(filePath);
    const isDir = stat.isDirectory();
    const mimeType = isDir ? null : mime.getType(filePath) || "application/octet-stream";
    const fileId = `${path.sep}${path.relative(this.baseDir, filePath)}`;
    const parentId = this.pathToParentFolder(fileId);
    return {
      type: isDir ? "directory" : "file",
      name: this.pathToFilename(filePath),
      id: fileId,
  ...
  public override async mkDir(name: string, baseId: string) {
    const dirPath = `${this.normalizePath(baseId)}/${this.normalizeFilename(name)}`;
    await fs.mkdir(dirPath);
    return this.toRemoteItem(dirPath);
  }

  public override async unlink(id: string) {
    const path = this.normalizePath(id);
  ...
  public override async readFile(id: string): Promise<[ReadStream, string]> {
    ...
    const filePath = this.normalizePath(id);
  ...
  public override async moveFile(
    ...
    if (deleteSource) {
      await fs.rename(this.normalizePath(id), destPath);
    }
    else {
      await fs.copyFile(this.normalizePath(id), destPath);
    }
