...
export async function initiate(profile: Profile, storageType: StorageType) {
    if (!envConfig.isStorageTypeEnabled(storageType) || !envConfig.isOneAuthEnabled()) {
        throw new Error('Storage is not enabled');
    }
    const apiUrl = joinUrlPath(envConfig.ONEAUTH_SERVER_URL!, '/api/session/initiate');
    ...
    const pendingAuth = await PendingAuth.createPendingAuth({ profile, storageType, referenceId, partialCode1 });
...
async function fetchAccessToken(apiKey: string): Promise<{
    ...
    const resp = await fetch(joinUrlPath(envConfig.ONEAUTH_SERVER_URL!, '/api/session/token'), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    return { accessToken, expiresOn: new Date(expiryDate), oneAuthId: id, storageType, targetId };
}
...
export async function complete(referenceId: string, partialCode2: string) {
    ...
    const { accessToken, expiresOn, oneAuthId, storageType } = await fetchAccessToken(secret);
    if (storageType !== pendingAuth.storageType) {
        console.error('Storage type mismatch', storageType, pendingAuth.storageType);
