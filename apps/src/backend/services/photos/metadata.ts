...
function getDate(dateStr: string, offset: string) {
    ...
    const [offsetHr, offsetMin] = offset.slice(1).split(':').map(x => parseInt(x));
...
export async function metaFromPhotoStream(stream: Readable) {
    ...
    if (tags.DateTimeOriginal && tags.OffsetTimeOriginal) {
        detail.capturedOn = getDate(tags.DateTimeOriginal.description, tags.OffsetTimeOriginal.description);
    }
    ...
    if (tags.ImageWidth && tags.ImageHeight && typeof tags.ImageWidth.value === 'number' && typeof tags.ImageHeight.value === 'number') {
...
export async function metaFromVideoStream(stream: Readable): Promise<any> {
    return new Promise((resolve, reject) => {
        ffmpeg(stream)
            .ffprobe(function (err, metadata: ffmpeg.FfprobeData) {
                if (err) {
                ...
                const detail: AssetDetailType = {
                    metadata: {
                        ...
                        fps: metadata.streams[0].r_frame_rate ? parseFloat(metadata.streams[0].r_frame_rate) : undefined,
                    },
                    height: metadata.streams[0].height,
                    width: metadata.streams[0].width,
                    duration: parseFloat(metadata.streams[0].duration || ''),
                    capturedOn: metadata.format.tags?.creation_time ? new Date(metadata.format.tags.creation_time) : new Date(),
                };
                resolve(detail);
            });
    });
